...
filt = "NIRCAM F182M"
rap = 62 * u.mas
eph = {"rh": 1.52 * u.au, "delta": 2.34 * u.au}  # ignoring phase effects
...
coma_rescaled = np.mean(
    np.mean(coma.reshape(200, 10, 2000), 1).reshape(200, 200, 10), 2
)
rho_rescaled = np.mean(
    np.mean(rho.reshape(200, 10, 2000), 1).reshape(200, 200, 10), 2
)

aperture = rho_rescaled <= rap.to_value("mas")
fluxd = coma_rescaled[aperture].sum() * (1 * u.mas) ** 2
print(
    "{} aperture from image (and ratio): {:.4f} ({:.4f})".format(
...
coma_rescaled2 = np.mean(
    np.mean(coma2.reshape(200, 10, 2000), 1).reshape(200, 200, 10), 2
)

fluxd = coma_rescaled2[aperture].sum() * (1 * u.mas) ** 2
print(
    "{} aperture from revised image (and ratio): {:.4f} ({:.4f})".format(
        rap, fluxd, fluxd / fluxd0
    )
)
# 0.0845 mJy (0.9920)
