...
parser = argparse.ArgumentParser(
    ...
    description=('Generate fixed targets for JWST APT based on moving target'
                 ' ephemerides.'),
...
parser.add_argument(
    'target', help='moving target, e.g., 1P, 24, P/2003 S2')
...
parser.add_argument('--type', choices=['comet', 'designation', 'smallbody', 'majorbody'],
                    default='smallbody', help='target type')
...
def get_ephemeris(args):
    ...
    horizons = Horizons(args.target, location='@jwst', epochs=epochs,
                        id_type=id_type)
...
def format_between(row):
    ...
    after = (row['date'] - 12 *
             u.hr).datetime.strftime('%d-%b-%Y:%H:00:00').upper()
    before = (row['date'] + 12 *
              u.hr).datetime.strftime('%d-%b-%Y:%H:00:00').upper()
    return after, before
...
def eph_to_table(eph):
    ...
    tab['Name'] = [
        f'''{target}.at.{row['date'].isot[:13]}'''
        for row in eph
    ]

    # reorder columns
    tab = tab['Name', 'RA', 'DEC', 'RA Uncertainty',
              'Dec Uncertainty', 'Comments']
    return tab
...
def eph_to_xml(eph):
    ...
    targets = root.find(
        'Targets', {None: 'http://www.stsci.edu/JWST/APT'}
    )
